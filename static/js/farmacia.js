...
function listarArquivosFarmacia() {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var files = JSON.parse(this.responseText);
            var fileList = document.getElementById("listarArquivosFarmacia");
            fileList.innerHTML = ""; // Limpa a lista antes de adicionar os novos arquivos
            if (files.length > 0) {
                files.forEach(function (file) {
                    ...
                    link.onclick = function() {
                        baixarArquivoFarmacia(file.url, file.filename);
                        return false; // Impede a navegação padrão ao clicar no link
                    };
                    listItem.appendChild(link);
                    fileList.appendChild(listItem);
                });
                Spopup(); // Exibe o popup com a lista de arquivos
            } else {
                alert("Nenhum arquivo encontrado.");
            }
        }
    };
    xhttp.open("GET", "/visualizarDocumentos", true);
    xhttp.send();
}

function baixarArquivoFarmacia(url, filename) {
    var link = document.createElement('a');
    link.href = url;
    link.download = filename;
    link.click();
...
function Tpopup() {
    if (PopupOpen) {
        ...
        listarArquivosFarmacia(); // Lista os arquivos antes de exibir o popup
        Spopup();
    }
}
...
function Spopup() {
    document.getElementById("Popup").style.display = "block";
    document.getElementById("botaoFechamento").style.display = "block";
    PopupOpen = true;
}

function Hpopup() {
    document.getElementById("Popup").style.display = "none";
    document.getElementById("botaoFechamento").style.display = "none";
    PopupOpen = false;
}

// Adicionando um event listener para o overlay
document.getElementById("botaoFechamento").addEventListener("click", function () {
    Hpopup(); // Fecha o popup quando o overlay é clicado
});

// Chamada inicial para listar os arquivos ao carregar a página
listarArquivosFarmacia();
